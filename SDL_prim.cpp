                            ...
                            Uint8 alpha)
{
	...
	if ( (p = SDL_getPixel(surf, x, y)) ) {
		SDL_GetRGBA(clr, surf->format, &r, &g, &b, &a);
		SDL_GetRGBA(*p, surf->format, &r2, &g2, &b2, &a2);

		r = r2 + (int)((double)( r - r2 ) * ( (double)alpha / 255.0 ));
		g = g2 + (int)((double)( g - g2 ) * ( (double)alpha / 255.0 ));
		b = b2 + (int)((double)( b - b2 ) * ( (double)alpha / 255.0 ));
		a = a2 + (int)((double)( a - a2 ) * ( (double)alpha / 255.0 ));

		clr = SDL_MapRGBA(surf->format, r, g, b, a);
		...
		if (flags & SDL_TG_FILL) {
			...
			if (alpha < 255) {
				for (fy = y - 1; fy >= (x ? x : 1); fy--) {
					if (x) {
						SDL_blendPixel(surf, x1 - x, (int)(y1 + fy), clr, alpha);
						SDL_blendPixel(surf, x1 - x, (int)(y1 - fy), clr, alpha);
					}
					SDL_blendPixel(surf, x1 + x, (int)(y1 + fy), clr, alpha);
					SDL_blendPixel(surf, x1 + x, (int)(y1 - fy), clr, alpha);
					...
					if (fy > x) {
						if (x) {
							SDL_blendPixel(surf, (int)(x1 - fy), y1 + x, clr, alpha);
							SDL_blendPixel(surf, (int)(x1 + fy), y1 + x, clr, alpha);
						}
						SDL_blendPixel(surf, (int)(x1 - fy), y1 - x, clr, alpha);
						SDL_blendPixel(surf, (int)(x1 + fy), y1 - x, clr, alpha);
				...
				for (fy = y - 1; fy >= (x ? x : 1); fy--) {
					if (x) {
						SDL_putPixel(surf, x1 - x, (int)(y1 + fy), clr);
						SDL_putPixel(surf, x1 - x, (int)(y1 - fy), clr);
					}
					SDL_putPixel(surf, x1 + x, (int)(y1 + fy), clr);
					SDL_putPixel(surf, x1 + x, (int)(y1 - fy), clr);
					...
					if (fy > x) {
						if (x) {
							SDL_putPixel(surf, (int)(x1 - fy), y1 + x, clr);
							SDL_putPixel(surf, (int)(x1 + fy), y1 + x, clr);
						}
						SDL_putPixel(surf, (int)(x1 - fy), y1 - x, clr);
						SDL_putPixel(surf, (int)(x1 + fy), y1 - x, clr);
                         ...
                         int x3, int y3, Uint32 clr, Uint8 alpha, Uint8 flags)
{
	...
	{ \
		if (fabs(SLOPE) > 1.0) { \
			c = (PB) > (B) ? -1 : 1; \
			if (flags & SDL_TG_ANTIALIAS) { \
				aastep = (float)alpha / fabs(SLOPE); \
				aa = (float)alpha; \
			} \
			for (y = (int)(PB); ( c > 0 ? y < floor(B) : (float)y > (B) ); y += c) { \
				if (alpha < 255) \
					SDL_blendPixel(surf, x, y, clr, (unsigned char)alpha); \
				else \
					SDL_putPixel(surf, x, y, clr); \
				if (flags & SDL_TG_ANTIALIAS) { \
			...
			if (alpha < 255) \
				SDL_blendPixel(surf, x, (int)(B), clr, (unsigned char)alpha); \
			else \
	...
	{ \
		if (fabs(SLOPE) > 2.0) { \
			...
			for (y = (int)((PB) + c); ( c > 0 ? y < floor(B) : (float)y > (B) ); y += c) { \
